...
| 경로 | 메서드 | 설명 |
|------|--------|------|
| `/solve` | POST | 문제 풀이 (메인 API) |
| `/health` | GET | 서버 상태 확인 |
| `/schema` | GET | 입출력 JSON 스키마 반환 (연동 참고용) |
| `/` | GET | 웹 GUI |
...
| `image` | string | X | base64 인코딩 이미지 |
| `mime_type` | string | X | 이미지 MIME 타입 (기본: image/png) |
| `rag_context` | string[] | X | n8n RAG 노드에서 보내주는 강의자료 |
| `rag_enabled` | bool | X | RAG 주입 on/off (기본: true) |
...
**구현:**
- `server.py` — `rag_context`/`rag_enabled` 필드 추가, `/schema` 엔드포인트 추가
- `gemini_client.py` — `solve_with_rag()` 함수 추가 (강의자료 프롬프트 주입)
- `n8n/workflow_template.json` — 기본 워크플로우 (n8n Import용)
- `n8n/workflow_with_rag.json` — RAG 포함 워크플로우 템플릿

**검증 완료:** curl로 텍스트 풀이, RAG 포함 풀이 모두 정상 동작 확인
...
- [ ] 웹검색 등 추가 MCP 서버 연결
- [ ] n8n에서 실제 벡터DB 연동 테스트 (Pinecone, Qdrant 등)
- [ ] Android APK 개발 (Kotlin/Jetpack Compose)
- [ ] Google Cloud e2-micro 배포
